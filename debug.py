...
def main():
    ...
    with connection:
        ...
        if count_of_not_unique:
            ...
            print('')
            print('WARNING!')
            print('There are {} not unique hashes in the DB'.format(
                count_of_not_unique))
            print(
                'It is happens because sometimes tenhou return content that belongs to other log')
            s = ','.join(["'{}'".format(x) for x in not_unique_hashes])
            cursor.execute(
                'UPDATE logs set is_processed = 0, was_error = 0, log_hash="", log_content="" where log_hash in ({});'.format(s))
            print('{} records were added to the download queue again'.format(
                count_of_not_unique))
            print('')
